        ...
        for (dynamic_covariate in names(x@data_dynamic)) {
          ...
          if (
            ...
            if (validation_fold > 0) {
              ...
              if ("include_clusters" %in% names(list(...)) && list(...)$include_clusters) {
                newdata <- newdata |> left_join(x@data_dynamic[[dynamic_covariate]] |> filter(get(x@times) <= landmarks), by = x@ids)
              }
