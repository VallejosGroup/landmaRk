...
#'
#' @returns A summary of the desired submodel, printed to the console.
...
setMethod("getEvent", "LandmarkAnalysis", function(object) {
  object@event_indicator
})
  ...
  function(x, landmarks, .warn_when_less_than = 0, ...) {
    ...
    if (length(landmarks) == 1) {
      ...
      x@risk_sets[[as.character(landmarks)]] <-
        x@data_static[[x@ids]][which(
          x@data_static[[x@event_time]] > landmarks
        )]
