...
Given a time-to-event outcome $T_i$, a landmark time $s$ and a time horizon $s + w$, the goal of a landmarking analysis is to estimate 
$$
\pi_i(s+w \vert s) = P(T_i > s+w \vert T_i \ge s, X_i(s)), 
$$
where $X_i(s)$ denotes a vector of covariates which may include time-varying covariates.
...
For the second component, at present the `landmaRk` package supports Cox proportional hazard models
as implemented in the `survival`package. 
...
Then, we can make predictions using the funciton `predict_survival`. Arguments
